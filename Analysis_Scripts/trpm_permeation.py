...
for protein in protein:
    ...
    for ion_solution in ion_solution:
        ...
        for replicate in replicate:
            ...
            df_name = f"gates_relative_coordinates_{protein}_{construct}_{ion_solution}_{replicate}.csv"
            ...
            for ion, s in zip(ionic_species,range(len(ionic_species))):
                ...
                ion_resid_file = f"permeating_ion_resids_{protein}_{construct}_{ion_solution}_{ionic_species[s]}_{replicate}.csv"
                ...
                np.savetxt(f"{result_dir}/permeation_times_{protein}_{construct}_{ion_solution}_{ionic_species[s]}_{replicate}.csv", sorted(permeating_time_ion[s]), delimiter=",", fmt='%s', header='Permeation_times') 

                print(f"Number of permeating {ionic_species[s]} ions = {len(permeating_time_ion[s])}")

                # Saving the permeation duration times...
                np.savetxt(f"{result_dir}/permeation_duration_{protein}_{construct}_{ion_solution}_{ionic_species[s]}_{replicate}.csv", sorted(permeating_length[s]), delimiter=",", fmt='%s', header='Permeation_duration') 
            ...
            for df, s in zip(df_list,range(len(ionic_species))):
                ...
                df_name = f"permeation_identification_pbc_{protein}_{construct}_{ion_solution}_{ionic_species[s]}_{replicate}.csv"
                df_path = os.path.join(result_dir,df_name)
